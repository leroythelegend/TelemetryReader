...
class TelemetryLobbyInfoPacket: TelemetryPacket {
    ...
    override init(data iter: inout Data.Iterator) throws {
        ...
        let numberOfPlayers: [TelemetryNumberOfPlayers] = try createTelemetryData(data: &iter)
        self.telemetryPackets["NUMPLAYERS"] = numberOfPlayers

        let lobbyInfo: [TelemetryLobbyInfo] = try createTelemetryData(data: &iter, size: NumberOfParticipants)
